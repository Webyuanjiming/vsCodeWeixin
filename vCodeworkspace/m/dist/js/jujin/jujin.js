...
app.config(function($routeProvider) {
    ...
    //金豆规则
    $routeProvider.when('/goldrule', {
        templateUrl: '/m/jujin/goldrule.html',
        controller:'ruleController'
    });
    //金豆记录
    $routeProvider.when('/goldrecord', {
        templateUrl: '/m/jujin/goldrecord.html',
        controller:'goldController'
    });
    //金豆中转页面
    $routeProvider.when('/transfer', {
        templateUrl: '/m/jujin/transfer.html',
    ...
    //账户
    $routeProvider.when('/defaultInfo', {
        templateUrl: '/m/jujin/account/waccountinfo.html',
        controller: 'waccountController'
    });
    //余额生息
    ...
    //自动投标详情说明
    $routeProvider.when('/autodeatil', {
        templateUrl: '/m/jujin/invest/autodeatil.html',
        controller: 'AutoInvestController'
    });
    //资金管理
...
/**-绑卡解绑中间的回调页面-**/
    $routeProvider.when('/cardsuccess',{
        templateUrl: '/m/jujin/cardsuccess/cardsuccess.html',
        controller: 'bankCtrl'
    });
});
...
//标的详情
app.controller('borrowinfoController', ['$rootScope', '$scope', '$routeParams','$location',"$sce",'$interval','commonService', 'GlobalBorrowInfo',
    function($rootScope, $scope, $routeParams,$location,$sce,$interval,service, borrowCache) {
        ...
        //显示倒计时
        function showCountdown(start,end){
            ...
            //投资状态描述
            $scope.statusMemo = hour+"时"+min+"分"+second+"秒后开始";
            //是否可投资
            $scope.isDisabled = true;
        }              
        //显示投资状态
        function showStatusMemo(systemTime,beginTime){
            //将"-"替换成"/"
            systemTime = systemTime && systemTime.replace(/-/g,"/");
            beginTime = beginTime && beginTime.replace(/-/g,"/");
            if(!isEmpty(systemTime) && !isEmpty(beginTime) && new Date(systemTime) < new Date(beginTime)){
                //显示倒计时
                showCountdown(systemTime,beginTime);
                var timer = $interval(function(){
                    //减一秒
                    var start = new Date(systemTime).getTime()+1000;
                    systemTime = start;
                    var end = new Date(beginTime).getTime();
                    if(new Date(start) < new Date(end)){
                        ...
                        //投资状态描述
                        $scope.statusMemo =  $scope.borrow.statusMemo;
                    }
                },1000);  
            }else{
                //是否可投资
        ...
        function loadBorrowAccountInfo() {
            service.loadAction({
                ...
                //系统时间
                var systemTime = $scope.borrow.systemTime;
                //开始时间
                var beginTime = $scope.borrow.beginTime;
                //倒计时-筹款中
                showStatusMemo(systemTime,beginTime);
                //转换认证资料
        ...
        $scope.invest = function() {
            ...
            if (!isEmpty($scope.borrow)) {
                ...
                //收益处理方式
                if($scope.recoverType == true){
                    investEntity.recoverType = '1';
                }else{
                    investEntity.recoverType = '0';
                }              
                //数据校验
                if (!isEmpty(investEntity.tenderAccount)) {
                    ...
                    service.loadRecord({
                        ...
                        if(data.status == true){
                            if (handlerReturnUrl($scope.returnUrl)) {
                                return;
                            } else if (handlerReturnUrl(data.returnUrl)) {                                
                                return;
                            }
                            service.loadAction({
                                ...
                                //是否购买过
                                $scope.verifyFlag =response.verifyFlag;           
                            });
                            //投资成功
                            $scope.investSuccess = true;
                            //计算投资利息
                            if($scope.borrow){
                                ...
                                var ticketValue = $scope.ticketValue||0;
                                //利息比率
                                var rate = (convertToFloat(borrow.borrowRate)+convertToFloat(ticketValue))/100;
                                var interest = convertToFloat($scope.amount)*rate/num*convertToFloat(borrow.period)+convertToFloat($scope.useCoinValue);
        ...
        function genShareInfo(){
            var userId = $scope.account && $scope.account.userId;
            service.loadRecord({
                url: $rootScope.URL_ROOT + "/shareactivity/tend?userId="+userId+"&money="+$scope.interest
            }).success(function(data) {
                $rootScope.loading = false;
                if(data.status == true){
                    var entity = data.entity;
                    var sentence = entity && entity.sentence;
                    $scope.sentence = sentence && sentence.split("_");
                    $scope.wxdata = entity;
                }
            });
        }
        ...
        $scope.ShowJujinBi = function() {
            ...
            coin = coin;
            //计算使用全部抵现红包需投金额
            $scope.useTotalTender = (coin/vouRate).toFixed(2);
            //支持抵现红包且有抵现红包
        ...
        //选择加息券
        $scope.checkTicket = function(ticket){
            $scope.tacketName = ticket.name; 
            $scope.ticketId = ticket.ticketId;
            $scope.ticketValue = ticket.ticketValue;
            if($scope.selectTicket() != false){
        ...
        $scope.selectTicket = function(){
            ...
            //最大值
            var maxAmount = ticket.maxAmount;
            //最小值
            var minAmount = ticket.minAmount;
            var borrow = $scope.borrow;
            //期限
            if(period && (borrow.isNew ==1 || period.indexOf(borrow.period) == -1)){
                $rootScope.Ui.turnOn('showinfo');
                $scope.msg = "该加息券只能用于"+ period + "月标，请重新选择！";
                $scope.title = "提示";
                $scope.ticketId = "0";
                return false;
            }
            ...
            if(minAmount>0 && maxAmount>0){
                if($scope.amount<minAmount || $scope.amount>maxAmount){
                    $rootScope.Ui.turnOn('showinfo');
                    $scope.msg = "该加息券投资金额需在" + minAmount + "-" + maxAmount + "元之间，请重新选择！";
                    $scope.title = "提示";
                    $scope.ticketId = "0";
                    return false;
                }
            }else if(minAmount>0){
                if($scope.amount<minAmount){
                    $rootScope.Ui.turnOn('showinfo');
                    $scope.msg = "该加息券最低投资金额为" + minAmount +"元，请重新选择！";
                    $scope.title = "提示";
                    $scope.ticketId = "0";
                    return false;
                }
            }else if(maxAmount>0){
                if($scope.amount>maxAmount){
                    $rootScope.Ui.turnOn('showinfo');
                    $scope.msg = "该加息券最高投资金额为" + maxAmount +"元，请重新选择！";
                    $scope.title = "提示";
                    $scope.ticketId = "0";
                    return false;
                }
    ...
    return{
        ...
        controller:["$rootScope","$scope","$http",function($rootScope,$scope,$http){
            ...
            function ShareInvite(data) {
                var wxdata = {
                    title : '30天年化收益13.2%',
                    imgUrl : "https://m.jujinziben.com/m/images/ic_launcher.png",
                    link : $scope.url,
                    desc : '新用户首投享受30天13.2%年化收益'
                };
                wx.config({
                    debug: false,
...
//邀请人列表
app.controller('inviteController', ['$rootScope','$scope', '$routeParams','$timeout','commonService', 'AccountInfo',
    function($rootScope, $scope, $routeParams,$timeout,service,account) {
        //初始化信息
        setDefaultValue($scope, $rootScope);
        //二维码图片
        ...
        //默认头像
        var defaultImg = "/m/dist/images/headpic.png";
        //加载更多
        $scope.loadMore = function() {
            if ($scope.currentPage < $scope.totalPageCount) {
                $scope.currentPage = $scope.currentPage + 1;
            ...
            if (isEmpty($scope.invites)) {
                service.loadPageAction(inviteConfig,$scope).success(function(response){
                    if(response.entity){
                        ...
                        //总条数
                        $scope.total = response.entity.invets && response.entity.invets.total;
                    }                    
                    if (!isEmpty(response.entity.invets.pageCount)) {
                        $scope.totalPageCount = response.entity.invets.pageCount;
                    }
        ...
        $scope.showCode = function(){
            $scope.isShowCode = !$scope.isShowCode;
        }
        //帮助说明
        ...
        //if(account.data){
            service.loadAction({
                url: $rootScope.URL_ROOT + "/default"
            },$scope).success(function(response){
                if(response.entity){
                    //账户信息
                    account.data = response.entity;
                    var account2 = response.entity.account;
                    if(account2){
                        //用户名
                        $scope.userId = account2.user && account2.user.userId;
                        //头像
                        $scope.userHeadImage = (account2.user && account2.user.userHeadImage) || defaultImg;
                    }                   
                }                                        
            });
        //}
...
app.controller('rechargeController', ['$rootScope', '$scope', '$http', '$location',
    function($rootScope, $scope, $http, $location) {
        ...
        $http.get($rootScope.URL_ROOT + '/bank')
            .success(function(responseData) {
                $scope.bankRt = responseData.entity;
                            ...
                            .success(function(responseData) {
                                if(responseData.status==true){
                                    ...
                                    var parameter={
                                        ...
                                        "userId":$scope.userBank,
                                        "platform":"3",
                                        "amount":$rootScope.amout
                                        /*"bankId":$scope.bankRt.bankCode*/
                                    };
                                    $http.post(bankUrl,parameter).success(
                                        function(data){
                                            $scope.moreStatus=data.entity;
                                            $("#tue").val($scope.moreStatus.Signature);
                                            $("#plan").val($scope.moreStatus.Plain);
                                            $("#mre").click();
                                        }
                                    );
                                }
    ...
    function($rootScope,$scope,$http,$routeParams,service){
        ...
        service.loadAction({
            ...
            if(validurl != undefined && validurl != ""){      
                ...
                //获取参数
                var urlList = validurl.split("?");
                if(urlList.length>1){
                    $scope.params = urlList[1];
                }
                //默认全部
        ...
        $scope.exchange = function(){
            ...
            var url = auth_url + "/redeem/exchange?callback=JSON_CALLBACK&code="+code+"&"+$scope.params;
            //优惠码兑换
        ...
        function genCodeListByType(type,entityName){
            if($scope[entityName] == undefined){
                //获取优惠码url
                var url = auth_url + "/redeem/"+type+"?callback=JSON_CALLBACK&"+$scope.params;
                $http.jsonp(url).success(function(response){
                    //验证返回值信息
                    checkResponse($rootScope, $scope, response);
                    //更新列表数据
                        ...
                        $scope.igr = function (timesMon) {
                            ...
                            function FormatDate(strTime) {
                                ...
                                if(date.getMonth().length=1){
                                    var dateMon="0"+(date.getMonth()+1);
                                    return date.getFullYear() + "-" + (dateMon+ 0) + "-" + date.getDate();
                                }else if(date.getMonth().length=2){
                                    //当长度为2
                                    var dateMon=(date.getMonth()+1);
                                    return date.getFullYear() + "-" + (dateMon) + "-" + date.getDate();
                                }
                            }
...
app.controller('withdrawController', ['$rootScope', '$scope', '$http', '$routeParams', '$location', '$sce',
    function($rootScope, $scope, $http, $routeParams, $location, $sce) {
        ...
        $http.get($rootScope.URL_ROOT + '/bank')
            .success(function(responseData) {
                $scope.bkRt = responseData.entity;
            });

        $scope.withdraw = function() {
            if (convertToFloat($scope.amount) <= 0 || convertToFloat($scope.amount) < 100) {
                $rootScope.Ui.turnOn('showinfo');
                        ...
                        $http.get($rootScope.URL_ROOT + '/atteststatus')
                            .success(function(responseData) {
                                if(responseData.status==true){
                                    ...
                                    var bankUrl=$rootScope.URL_ROOT+"/accountManagePlat/mobileAccountManage";
                                    var parameter={
                                        "service":"withdrawDeposit",
                                        "userId":$scope.userBank,
                                        "platform":"3",
                                        "amount":$scope.amount,
                                        "bankId":$scope.bkRt.bankCode
                                    };
                                    $http.post(bankUrl,parameter).success(
                                        function(data){
                                            $scope.moreStatus=data.entity;
                                            $("#tue").val($scope.moreStatus.COMMONVAL);
                                            /*                                        $("#tue").val($scope.moreStatus.Signature);
                                             $("#plan").val($scope.moreStatus.Plain);*/
                                            $("#w_draw").click();
                                        }
...
//绑定提现银行卡
app.controller('bankcardController',['$rootScope', '$scope', '$http', '$routeParams', '$location', function($rootScope, $scope, $http, $routeParams,$location) {
    $scope.bakFirst=function(){
        $http.get($rootScope.URL_ROOT + '/atteststatus')
            .success(function(responseData) {
                    if(responseData.status==true){
                        if(responseData.entity.idFlg==0||responseData.entity.emailFlg==0){
                            $location.url("/attest");
                        }
                        if(responseData.entity.signFlag==0){
                                        $scope.userBank=responseData.entity.userId;
                                        var bankUrl=$rootScope.URL_ROOT+"/accountManagePlat/mobileAccountManage";
                                        var parameter={
                                            "service":"signClient",
                        ...
                        }else if(responseData.entity.signFlag==1){
                            $scope.userBank=responseData.entity.userId;
                            var bankUrl=$rootScope.URL_ROOT+"/accountManagePlat/mobileAccountManage";
                            var parameter={
                                "service":"bindCard",
...
//银行卡绑定和解绑回调
app.controller('bankCtrl', ['$scope', '$location', '$rootScope', '$http','$routeParams',
    function($scope, $location, $rootScope, $http,$routeParams) {
        $scope.addMoney=false;
        $scope.dateSuccess=false;
        $scope.addTwo=false;
        $scope.dateTwo=false;
        if($routeParams.service=='bindCard'){
            $scope.msgService="绑卡";
        }else if($routeParams.service=='signClient'){
            $scope.msgService="签约";
        }
        /**绑卡* && $routeParams.platform=='3'||'2'||'1'||'4'*/
        if($routeParams.status=='true' && $routeParams.service=='bindCard'){
            $scope.addSuccess=true;
            $scope.addError=false;
            $scope.imgReturn=true;
            $scope.errReturn=false;
            $scope.successReturn=true;
            $scope.ssReturn=false;
            if($routeParams.platform=='3'){
                $scope.successReturn=true;
                $scope.ssReturn=false;
            }else if($routeParams.platform=='1'||$routeParams.platform=='2'){
                $scope.ssReturn=true;
                $scope.successReturn=false;
            }
            $scope.successFirst=function(){
                $location.url("/bankcard");
            }
        }else if($routeParams.status=='false' && $routeParams.service=='bindCard'){
            $scope.addError=true;
            $scope.addSuccess=false;
            $scope.imgReturn=false;
            $scope.errReturn=true;
            $scope.successReturn=true;
            $scope.ssReturn=false;
            if($routeParams.platform=='3'){
                $scope.successReturn=true;
        ...
        /**签约**/
        if($routeParams.status=='true' && $routeParams.service=='signClient'){
            $scope.addSuccess=true;
            $scope.addError=false;
            $scope.imgReturn=true;
            $scope.errReturn=false;
        ...
        }else if($routeParams.status=='false' && $routeParams.service=='signClient'){
            $scope.addError=true;
            $scope.addSuccess=false;
            $scope.imgReturn=false;
            $scope.errReturn=true;
        ...
        if($routeParams.service=='unwrapBankCard'){
            $scope.msgService="解绑";
        }
        if($routeParams.status=='true' && $routeParams.service=='unwrapBankCard'){
            $scope.addSuccess=true;
            $scope.addError=false;
            $scope.imgReturn=true;
            $scope.errReturn=false;
        ...
        }else if($routeParams.status=='false' && $routeParams.service=='unwrapBankCard'){
            $scope.addError=true;
            $scope.addSuccess=false;
            $scope.imgReturn=false;
            $scope.errReturn=true;
        ...
        /**-充值-**/
        if($routeParams.service=='yidongzhifu'){
            $scope.msgService="充值";
        }
        if($routeParams.status=='true' && $routeParams.service=='yidongzhifu'){
            var urlPlatform=$rootScope.URL_ROOT+"/accountManagePlat/mobilePayResult?platform="+$routeParams.platform;
            $http.get(urlPlatform).success(
                function(response){
                    $scope.ckResult=response.checkResult;
                    if(response.checkResult=='1'){
                        $scope.resultMon=response;
                        $scope.addSuccess=true;
                        $scope.addError=false;
                        $scope.imgReturn=true;
                        $scope.errReturn=false;
                    ...
                    }else if(response.checkResult=='0'){
                        $scope.addSuccess=false;
                        $scope.addError=true;
                        $scope.imgReturn=false;
                        $scope.errReturn=true;
                    ...
                    }else if(response.checkResult=='2'){
                        if($routeParams.platform=='3'){
                            $scope.tradeReturn=false;
                            $scope.trade=true;
                        }else if($routeParams.platform=='1'||$routeParams.platform=='2'){
                            $scope.tradeReturn=true;
                            $scope.trade=false;
                        }
                        $scope.successFirst=function(){
                            $location.url("/");
        ...
        /**-提现-**/
        if($routeParams.service=='withdrawDeposit'){
            $scope.msgService="提现";
        }
        if($routeParams.status=='true' && $routeParams.service=='withdrawDeposit'){
                    $scope.ckResult=1;
                    $scope.amountThr=$routeParams.amount;
                    $scope.dateThr=$routeParams.date;
                    $scope.hand=$routeParams.handleTime;
                    $scope.bankNe=$routeParams.bankName;
                    $scope.addSuccess=true;
                    $scope.addError=false;
                    $scope.imgReturn=true;
                    $scope.errReturn=false;
        ...
        }else if($routeParams.status=='false' && $routeParams.service=='withdrawDeposit'){
                    $scope.addSuccess=false;
                    $scope.addError=true;
                    $scope.imgReturn=false;
                    $scope.errReturn=true;
...
app.directive('pagteUrl', ['$rootScope','$routeParams','$location',function($rootScope,$routeParams,$location) {
    return {
        restrict: 'A',
        link: function(scope, element, attrs) {
                element.attr("href",'jujin://router?data={"action": "back","url": "","flag": ""}');
        }
    }
}]);
//收益计算器
...
app.controller('HomeController', ['$rootScope', '$scope', '$routeParams', '$location',
    function($rootScope, $scope, $routeParams, $location) {
        ...
        $rootScope.ShowBottomNav = function() {
            ...
            /**-存管跳转-**/
            else if(path.indexOf("cardsuccess") != -1){
                return false;
            }
            /**-金豆-**/
            else if(path.indexOf("goldrule") != -1){
                return false;
            }
            else if(path.indexOf("goldrecord") != -1){
                return false;
            }
            return true;
        }
...
/**-金豆规则-**/
app.controller('ruleController', ['$rootScope', '$scope', '$sce','$http',
    function($rootScope, $scope, $sce,$http) {
        var ruleUrl=$rootScope.URL_ROOT+"/goldbean/page";
        $http.get(ruleUrl).success(
            function(ent){
                $scope.helpUrt=ent.entity.help;
                $("#gold_txt").html($scope.helpUrt);
            }
        );
    }
]);
/**-金豆中转页面-**/
app.controller('transferController', ['$rootScope', '$scope', '$sce','$http',
    function($rootScope, $scope, $sce,$http) {
        var transferUrl=$rootScope.URL_ROOT+"/goldbean/page";
        //排序比较id比较函数
        ...
        $http.get(transferUrl).success(function(data){
            $scope.awards=data.entity.award;
            $scope.goldbean=data.entity.goldbean;
            $scope.home=data.entity.home;
            $scope.awards.sort($scope.compare("id"));
            var awardAll=[];
            for(var i=0,len=6;i<len;i++){
                $scope.awardsImg=$scope.awards[i];
                awardAll.push($scope.awardsImg);
            }
            $scope.awardAll=awardAll;
        });
    }
]);
//金豆记录goldController
app.controller('goldController', ['$rootScope', '$scope', '$sce','commonService',
    function($rootScope, $scope, $sce,service) {
        ...
        var logConfig = {
            url : $rootScope.URL_ROOT + "/goldbean/get"
        };
        //加载（获奖记录）更多
        $scope.loadMore = function() {
        ...
        //获取(使用记录)的数据参数
        var logConfig2 = {
            url : $rootScope.URL_ROOT + "/goldbean/use"
        };
        //加载（使用记录）更多
        $scope.loadMore2 = function() {
            service.loadMore(logConfig2,$scope,{
                proName: "use"
            },formatAccountLog,$sce);
        };
        $scope.reloadData=function(dater){
            if(dater=='1'){
                //未加载过数据时加载数据
                if (isEmpty($scope.gold)) {
                    $scope.curPage =1;
            ...
            if(dater=='2'){
                //未加载过数据时加载数据
                if (isEmpty($scope.use)) {
                    service.genRecord(logConfig2,$scope,{
                        proName: "use"
                    },formatAccountLog,$sce);
               }
            }
    ...
    $scope.login = function(args) {
        ...
        $http.post($rootScope.URL_ROOT + "/login", $scope.user).success(function(data, status) {
            if (checkResponse($rootScope, $scope, data)) {
                ...
                $rootScope.IS_LOGIN = true;
                if (handlerReturnUrl(data.returnUrl)) {
                    return;
                }
                ...
                $scope.depositFlg=data.depositFlg;
                $scope.sigFlag=data.signFlag;
                if($scope.depositFlg=="1"){
                    $location.url("/default");
                    localStorage.setItem("sitFlg",1);
                    localStorage.setItem("sigFlag",$scope.sigFlag);
                }else if($scope.depositFlg=="0"){
                    $location.url("/defaultInfo");
                    localStorage.setItem("sitFlg",0);
                    localStorage.setItem("sigFlag",$scope.sigFlag);
                }
                //登录正常跳转自用户中心页面
            } else {
...
//发送短信验证码新版-----zdq-2017-02-13
function SendSms2($rootScope, $http, $scope, tel, img) {
    ...
    var timer = setInterval(function() {
        if (time > 1) {
            ...
            $scope.$apply(function() {
                if(time<10){
                    $scope.btnmsg = '0' + time + 's 后重试';
                }else{
                    $scope.btnmsg = time + 's 后重试';
                }                           
                $scope.disable = false;
            });
        } else {
...
app.controller('registerController', function($rootScope, $scope, $http, $routeParams, $location) {
    ...
    $scope.getSmsCode = function() {
        ...
        if ($scope.disable) {
            ...
            if(!checkIsMobile(tel)){
                showMsgInfo("请输入正确的手机号码！");
                return false;
            }           
            //验证码
            if($scope.validate == 1 && !$scope.reg.imgVerifyCode){
               showMsgInfo("请输入验证码！"); 
               return false;
           }            
            //发送短信
            SendSms2($rootScope, $http, $scope, tel,$scope.reg.imgVerifyCode);
        }
    }
    ...
    $scope.register = function() {
        ...
        if(pwd.length < 6 || pwd.length > 16){
            showMsgInfo("请输入6-16位密码！");
            return false;
        }       
        //密码是否一致
        if(!$scope.norepsw && pwd!=repsw){
            showMsgInfo("前后两次密码不一致！");
            return false;
        }        
        //是否同意协议
        if(!$scope.reg.agree){
...
//注册
app.controller('registerController2', function($rootScope, $scope, $http, $routeParams, $location) {
    ...
    function($scope, $rootScope, $http, $location, $routeParams) {
        ...
        $scope.sendmsg = function(){
            if ($scope.disable) {
                ...
                //手机号格式是否正确
                if(!checkIsMobile($scope.form1.phone)){
                    $rootScope.Ui.turnOn('showinfo');
                ...
                //验证码
                if ($scope.validate == 1 && !$scope.form1.imgVerifyCode) {
                    $rootScope.Ui.turnOn('showinfo');
                    $scope.msg = "请输入下方验证码！";
                    $scope.title = "提示";
                    return false;
                ...
                //发送短信
                SendSms2($rootScope, $http, $scope, $scope.form1.phone,$scope.form1.imgVerifyCode);
                //获取用户名
                $http.get($rootScope.URL_ROOT + "/user/checkuserbytel?tel=" + $scope.form1.phone)                
                .success(function(responseData) {
                    if (checkResponse($rootScope, $scope, responseData)) {
                        //多用户并且选中第一个
                        if(!isEmpty(responseData.entity) && responseData.entity.length>0){
                            $scope.userList = responseData.entity;
                            $scope.userId = $scope.userList[0];
                        }                       
                    }
                });
            }            
        ...
        $scope.confirm = function(num) {
            ...
            if (num === 1) {
                ...
                //密码是否为6-12位
                if($scope.form2.psw && ($scope.form2.psw.length<6||$scope.form2.psw.length>16)){
                    $rootScope.Ui.turnOn('showinfo');
                ...
                //密码是否一致
                if($scope.form2.psw!=$scope.form2.repsw){
                    $rootScope.Ui.turnOn('showinfo');
                    $scope.msg = "前后两次密码不一致！";
                    $scope.title = "提示";
                    return false;
...
//vip管理
app.controller("vipController",["$rootScope","$http","$scope","$location","commonService",function($rootScope,$http,$scope,$location,service){
    //获取vip信息
    $scope.genVip = function(){
        service.loadAction({
            ...
            $scope.vipSlider=response.vipMenus;
            //是否购买过
            var verifyFlag =response.verifyFlag;
            //购买信息
            var startTimes=response.startDate.replace(/-/g,"/")||"";
            var endTimes=response.endDate.replace(/-/g,"/")||"";
            $scope.vipstatus = verifyFlag == "1" ? startTimes + "—" + endTimes:"无";
            for(var i=0,len=response.vipMenus.length;i<len;i++){
                if(response.vipMenus[i].recommendFlag == "1"){
                    $scope.active = response.vipMenus[i];
                    $scope.menuActive=response.vipMenus[i];
                    break;
                }
            }
        });
    ...
    $scope.showModel = function(){
        ...
        $scope.payCenter=false;
        $scope.payMsg=false;
    }
    service.loadAction({
        url: $rootScope.URL_ROOT + "/default"
    },$scope).success(function(response) {
        $scope.phoneMsg=response.entity.account.user.phoneNumber;
    });
    /***-短信验证码确认-***/
    $scope.btnModel=function(){
        $scope.payCenter=false;
        $scope.payMsg = !$scope.payMsg;
        $scope.showPay=false;
    }
    //短信验证码---------------------------------------------(修改成指令)-------------------
    $scope.btnmsg = "获取验证码";
    $scope.disable = true;
    $scope.modelMsg=function(){
        var msgUrl=$rootScope.URL_ROOT + "/bid/applySMS?amount="+$scope.active.vipMoney;
        $http.get(msgUrl).success(
            function(response){
                if(response.status==true){
                    $rootScope.Ui.turnOn('showinfo');
            ...
            if ($scope.disable) {
                $scope.disable = false;
                //发送短信
                //SendSms($rootScope,$http, $scope,$scope.phoneMsg);
                var timer = setInterval(function() {
                    if (time > 0) {
                        time--;
                        $scope.$apply(function() {
    ...
    $scope.msgCenter=function(){
        if($scope.sumItem){
            var goodsUrl=$rootScope.URL_ROOT + "/bid/buyGoods?msgCode="+$scope.MsgCode+"&vipId="+$scope.active.vipId+"&ticketId="+$scope.sumItem;
        }else{
            var goodsUrl=$rootScope.URL_ROOT + "/bid/buyGoods?msgCode="+$scope.MsgCode+"&vipId="+$scope.active.vipId;
        }
        $http.get(goodsUrl).success(
            function(response){
                if(response.status==true){
...
//                    return;
    ...
    $scope.buyVip = function(){
        ...
        }else if($scope.itNew=="1"){
            var dataSum=
            {
                "cashPwd": $scope.cashPwd,
                "vipId": $scope.active.vipId,
                "ticketId":$scope.sumItem
            };
        }
        service.loadAction({
    ...
    $scope.vTicket=function(){
        service.loadAction({
            url: $rootScope.URL_ROOT + "/certification/listVipCoupon"
        },$scope).success(function(response) {
            $scope.couponList=response.entity;
    ...
    $scope.cancel=function(){
        $scope.itNew="0";
        $scope.benefit=false;
        $scope.vipMenus=$scope.vipSlider;
        $scope.active=$scope.menuActive;
    }
    $scope.getPrice=function($id){
        service.loadAction({
            url:$rootScope.URL_ROOT + "/certification/getVipPrice?id="+$id
        },$scope).success(function(response) {
            $scope.vipMenus=response.entity;
           for(var i=0,len=response.entity.length;i<len;i++){
               // if(response.entity[i].recommendFlag == "1"){
                $scope.entActive = response.entity[i];
               if($scope.entActive.vipPeriod==$scope.menuActive.vipPeriod){
                   $scope.active=$scope.entActive;
                  break;
               }
            }
            $scope.showPay = false;
    ...
    $scope.selectPon=function($em){
        $scope.activeNem=$scope.couponList[$em];
        var idNem=$scope.couponList[$em].couponId;
        $scope.sumItem=$scope.couponList[$em].couponId;
        $scope.scrollVip=true;
        $scope.scrVip=false;
        $scope.tomTxt=true;
        if(!isEmpty($scope.couponList[$em])){
            $scope.itNew="1";
            $scope.getPrice(idNem);
            $scope.benefit=true;
        }else{
            $scope.itNew="0";
        }
...
//任务列表
app.controller('taskController',["$rootScope","$scope","$location","commonService",function($rootScope,$scope,$location,service){   
    var pageSize = 5;
    //任务列表
    $scope.genTaskList = function(type){
        //任务参数
        var taskConfig = {
            url : $rootScope.URL_ROOT + "/task/list?type=" + type
        };
        service.loadPageAction(taskConfig,$scope).success(function(response){
            //任务列表
    ...
    //新增页签--所有任务
    $scope.genAllTask = function(){
        //任务参数
        var taskConfig = {
            url : $rootScope.URL_ROOT + "/task/listAll"
        };
        service.loadPageAction(taskConfig,$scope).success(function(response){
            //任务列表
            $scope.taskList = response.list;                     
    ...
    //加载更多任务列表
    $scope.loadMoreTask = function(type){
        //任务参数
        var url = $rootScope.URL_ROOT + "/task/list?type=" + type;
        loadMoreTaskByUrl(url);
    }
    function loadMoreTaskByUrl(url){
        //任务参数
        var taskConfig = {
            url : url,
            params:{
        ...
        if ($scope.currentPage < $scope.totalPageCount) {
            $scope.currentPage = $scope.currentPage + 1;
            //请求第几页
            taskConfig.params.pi = $scope.currentPage;
            service.loadPageAction(taskConfig,$scope).success(function(response){
                //任务列表
                var list = response.list;
                $scope.taskList = $scope.taskList.concat(list);                   
            });
        }
    }
    //加载更多所有任务列表
    $scope.loadMoreAllTask = function(){
        var url = $rootScope.URL_ROOT + "/task/listAll" ;
        loadMoreTaskByUrl(url);
    }
    //页签切换
    $scope.currentTab = 1;
    ...
    $scope.setActiveTab = function(tabNum){
        ...
        if(tabNum == 3){
            ...
            $scope.genTaskList(tabNum);
        }       
    }
    //领取奖励
    $scope.receiveAward = function(taskid){
        if(!taskid){
            return;
        }
        //领取奖励      
        service.loadAction({
            url: $rootScope.URL_ROOT + "/task/getAward?id="+taskid
        },$scope).success(function(response) {
           $scope.showAward = true;
           $scope.awardList = response.entity;
           $scope.genTaskList($scope.currentTab);
        });
    }
    //去完成
    $scope.doTask = function(type){
        switch(type){
            //2-绑手机
            case "2":
                $location.url("/attest");
            ...
            case "10":
                ...
                break;
            default:
            //1、投标（后跟",月份_月份_月份]"，也可不跟）例如：1,1_3_12表示要投1、3、12月标
                $location.url("/borrows");
        }    
    }
    //关闭奖励
...
//投资成功分享
app.directive('investShareUrl',[function(){
    return{
        restrict: "A",
        scope:{
            wxdata:"="
        },
        controller:["$rootScope","$scope","$http","commonService",function($rootScope,$scope,$http,service){
            //获取分享数据
            ...
            function shareSuccess(){
                service.loadAction({
                    url: $rootScope.URL_ROOT + "/shareactivity/getAward"
                },$scope).success(function(response) {
                    $("#investSusModel").show();
                });
            }
            function Share(data) {
                var wxdata = {
                    title : $scope.wxdata.title,
                    imgUrl : $scope.wxdata.imgUrl,
                    link : $scope.wxdata.url+"?award="+$scope.wxdata.award,
                    desc : $scope.wxdata.content
                };
                wx.config({
                    debug: false,
                    appId: "wx30235c7115ff82d5",
                    ...
                    nonceStr: data.noncestr,
                    signature: data.signature,
                    jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage','onMenuShareQQ','onMenuShareQZone','hideMenuItems' ] // 功能列表，我们要使用JS-SDK的什么功能
                });
            ...
            scope.getContentUrl = function(){
                ...
                if(scope.borrow.isNew == 1){
                    ...
                    switch (scope.borrow.type){
                        ...
                        //企业标产品介绍
                        case 13:
                            htmlurl = "/m/jujin/sc/enterprise.html";
                            break;
                        //车分期产品介绍
                        case 11:
