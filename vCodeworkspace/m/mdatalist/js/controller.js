...
app.directive('pagteUrl', ['$rootScope',function($rootScope) {
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            var borrowtitle=scope.pagte.BORROW_TITLE;
            var borrowtype=scope.pagte.BORROW_TYPE;
            var trabsferflg=scope.pagte.TRANSFER_FLG;
            //console.log(borrowid);
            if(mobileFlag == 1){
                element.attr("href",'jsbridge://action?data={"action":"borrow","flag":"可选","type": "'+borrowtype+'","borrowId":"'+borrowid+'","borrowTitle":"'+borrowtitle+'","transferFlg":"'+trabsferflg+'"}');
            }else{
            ...
            function(response){
                ...
                for(var i= 0,len=$scope.volume.length;i<len;i++){
                    if($scope.volume[i] != null){
                        ...
                        $scope.volume[i].repayAccountInterestWait = $scope.volume[i].repayAccountInterestWait.toFixed(2)+"";
